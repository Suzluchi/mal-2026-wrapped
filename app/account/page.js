...
export default async function Account() {
  ...
  return <main className="text-page"><Link className="brand" href="/">✳ MAL WRAPPED</Link><p className="eyebrow">CONNECTED TO MYANIMELIST</p><h1>Welcome, {session.user.name}.</h1><p>Your account is connected. Import your real anime and manga lists to inspect your titles, scores, and recorded dates.</p><p>You can still explore the fictional sample. It does not use your account data.</p><Link className="button primary" href="/lists">Import my lists ↗</Link><form action="/api/auth/logout" method="post"><button className="button secondary" type="submit" style={{ marginTop: 20 }}>Log out</button></form><p>This session lasts up to one hour. Logging out clears it on this browser.</p></main>;
}
