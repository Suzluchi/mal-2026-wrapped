...
function Summary({ kind, state, year }) {
  ...
  return <>
    ...
    {year !== 'all' && <p>{stats.started} titles have a recorded start date in {year}. {stats.undatedCompleted} completed titles have missing, partial, or invalid finish dates and are excluded from the yearly count.</p>}
...
export default function ListImporter() {
  ...
  return <>
    ...
    {lists[kind].status === 'done' && <details><summary>Explore your full {kind} list</summary><Summary kind={kind} state={lists[kind]} year={year} /></details>}
    </section>)}
  </>;
}
