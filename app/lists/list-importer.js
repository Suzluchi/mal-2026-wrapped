...
function Summary({ kind, state, year, asOf }) {
  ...
  return <>
    <div className="list-metrics"><div><strong>{stats.total}</strong><span>titles on your current list</span></div><div><strong>{stats.completed}</strong><span>currently marked completed</span></div>{year !== 'all' && <div><strong>{stats.finished.length}</strong><span>completed with a finish date in {year}</span></div>}</div>
    {year !== 'all' && <p>{stats.started} titles have a recorded start date in {year}. {stats.undatedCompleted} completed titles have missing, partial, or invalid finish dates and are excluded from the yearly count. {stats.issues.future} have future finish dates; {stats.issues.reversed} have a finish date before their start date. Those dates are also excluded.</p>}
...
export default function ListImporter({ asOf }) {
  ...
  return <>
    <div className="list-notice"><h2>Dates tell part of the story.</h2><p>MAL lists are a current snapshot, not a complete activity history. Yearly counts use complete finish dates on or before {asOf} (UTC), excluding dates before a recorded start date. All time counts titles currently marked completed, even when their dates need correction. They do not prove how many episodes or chapters you consumed in that year. Rewatches, rereads, and activity without dates are not reconstructed.</p><label>Recap period <select value={year} onChange={e => setYear(e.target.value === 'all' ? 'all' : Number(e.target.value))}><option value="all">All time</option>{Array.from({ length: Number(asOf.slice(0,4)) - 1899 }, (_, i) => Number(asOf.slice(0,4)) - i).map(y => <option key={y} value={y}>{y}</option>)}</select></label></div>
    {lists.anime.status === 'done' && lists.manga.status === 'done' && <PersonalRecap key={`recap-${year}`} anime={lists.anime.items} manga={lists.manga.items} period={year} asOf={asOf} />}
    {lists.anime.status === 'done' && lists.manga.status === 'done' && <ListInsights key={`insights-${year}`} anime={lists.anime.items} manga={lists.manga.items} period={year} asOf={asOf} />}
