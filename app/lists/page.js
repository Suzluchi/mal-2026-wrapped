...
export default async function Lists() {
  ...
  return <main className="lists-page"><Link className="brand" href="/account">MAL WRAPPED / MY ACCOUNT</Link><p className="eyebrow">YOUR PERSONAL RECAP</p><h1>Your stories, in review.</h1><p>Your anime and manga lists load automatically after sign-in. We only read your lists; we do not change them.</p><ListImporter asOf={new Date().toISOString().slice(0,10)} /><p><Link href="/privacy">Privacy</Link> · <Link href="/account">Back to account</Link></p></main>;
}
