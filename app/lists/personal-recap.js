...
export default function PersonalRecap({ anime, manga, period, asOf }) {
  ...
  slides.push({title:'A little context for your story.',body:<><p>{a.missingDates} completed anime and {m.missingDates} completed manga have missing, partial, or invalid finish dates. They count in All time, but cannot be placed in an annual recap or completion month.</p><p>{a.issues.future + m.issues.future} completed titles have future finish dates, and {a.issues.reversed + m.issues.reversed} have finish dates before their recorded start dates. They remain in All time but are excluded from yearly totals and completion months.</p><p>This is a snapshot of your current MAL lists. It cannot reconstruct rewatches, rereads, or time spent watching and reading.</p><p>You can change the period above or explore your full lists below.</p></>});
