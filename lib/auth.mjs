...
export async function callback(request, env = process.env, fetcher = fetch) {
  ...
  try {
    ...
    const value = seal({ user: { id: user.id, name: user.name }, accessToken: token.access_token, exp: Date.now() + seconds * 1000 }, 'session', cfg.secret);
    const response = finish('/account');
