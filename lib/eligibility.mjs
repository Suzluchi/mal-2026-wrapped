...
export function completionIssue(item, asOf = todayUTC()) {
  if (item.isRepeating === true) return 'activeRepeat';
...
export function selectCompletions(items, period, asOf = todayUTC()) {
  ...
  const completed = entries.filter(item=>item.status==='completed' || item.isRepeating===true);
  const issues = {undated:0,future:0,reversed:0,activeRepeat:0};
