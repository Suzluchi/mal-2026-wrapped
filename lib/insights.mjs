import { selectCompletions, todayUTC } from './eligibility.mjs';
...
export function coverURL(value) {
  try { const u=new URL(value); return u.protocol==='https:' && u.hostname==='cdn.myanimelist.net' && !u.username && !u.password && !u.port ? u.href : null; } catch { return null; }
}
export function normaliseMetadata(node, kind) {
  const length = kind==='anime' ? node.num_episodes : node.num_chapters;
  return { cover:coverURL(node.main_picture?.large) || coverURL(node.main_picture?.medium), length:Number.isSafeInteger(length) && length > 0 ? length : null, genres:named(node.genres), studios:kind==='anime'?named(node.studios):[],
...
export function insights(items, period, asOf = todayUTC()) {
  const {selected:eligible} = selectCompletions(items, period, asOf);
