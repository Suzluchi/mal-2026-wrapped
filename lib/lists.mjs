import { dateYear, selectCompletions, todayUTC } from './eligibility.mjs';
export { dateYear } from './eligibility.mjs';
...
export async function fetchListPage(token, kind, offset = 0, fetcher = fetch) {
  ...
  url.search = new URLSearchParams({ fields: `list_status{status,score,start_date,finish_date,${kind === 'anime' ? 'is_rewatching,num_times_rewatched' : 'is_rereading,num_times_reread'}},main_picture,genres,start_date,media_type,${kind === 'anime' ? 'studios,source,num_episodes' : 'authors{first_name,last_name},num_chapters'}`, limit: '100', offset: String(offset), nsfw: 'true' }).toString();
  let response;
  ...
  const items = payload.data.map(entry => {
    ...
    const repeatCount = kind === 'anime' ? s.num_times_rewatched : s.num_times_reread;
    const repeatFlag = kind === 'anime' ? s.is_rewatching : s.is_rereading;
    return { ...normaliseMetadata(entry.node, kind), id: entry.node.id, title: entry.node.title, repeatCount:Number.isSafeInteger(repeatCount) && repeatCount >= 0 ? repeatCount : null, isRepeating:typeof repeatFlag === 'boolean' ? repeatFlag : null, status: s.status || 'unknown',
...
export function summarise(items, year, asOf = todayUTC()) {
  const {entries,completed,selected,issues}=selectCompletions(items,year,asOf);
  return {total:entries.length,completed:completed.length,finished:selected,undatedCompleted:issues.undated,issues,
    started:entries.filter(item=>dateYear(item.startDate)===year && item.startDate<=asOf).length};
}
