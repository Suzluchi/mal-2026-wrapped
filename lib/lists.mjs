...
export async function fetchListPage(token, kind, offset = 0, fetcher = fetch) {
  ...
  url.search = new URLSearchParams({ fields: 'list_status{status,score,start_date,finish_date}', limit: '100', offset: String(offset), nsfw: 'true' }).toString();
  let response;
  ...
  const items = payload.data.map(entry => {
    ...
    return { id: entry.node.id, title: entry.node.title, status: s.status || 'unknown',
