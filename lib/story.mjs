...
export function buildStory(anime,manga,period,asOf) {
 ...
 for(const [key,items] of [['anime',anime],['manga',manga]]) {
  ...
  if(!s.count)continue;
  add('count','count',a?'anime completed. But where did you find the time?':'manga completed.',{eyebrow:a?'':'THE PAGES WERE TURNING.',value:s.count,quip:a?'“One more episode” really added up.':'“Just one more chapter” was clearly a lie.'});
  if(s.rated){
